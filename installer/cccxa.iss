...
; userdesktop/userprograms (not auto*): even though the install runs elevated as admin, the
; shortcuts must land only on the installing user's own Desktop / Start Menu - not the common
; (all-users) locations, so monitored users never see a dashboard shortcut.
[Icons]
Name: "{userdesktop}\cccxa - Activity Dashboard";           Filename: "{app}\cccxa.exe"; Parameters: "serve"; WorkingDir: "{app}"; IconFilename: "{sys}\shell32.dll"; IconIndex: 171; Tasks: desktopicon
Name: "{userprograms}\cccxa\cccxa - Activity Dashboard";    Filename: "{app}\cccxa.exe"; Parameters: "serve"; WorkingDir: "{app}"; IconFilename: "{sys}\shell32.dll"; IconIndex: 171; Tasks: startmenu
Name: "{userprograms}\cccxa\Uninstall cccxa";              Filename: "{uninstallexe}"; Tasks: startmenu
