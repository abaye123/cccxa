...
var overrideDir = Path.GetDirectoryName(overrideConfig)!;
var overrideName = Path.GetFileName(overrideConfig);

// חשוב: overrideConfig הוא נתיב מוחלט (ProgramData), אבל ספק הקבצים של ה-ConfigurationBuilder
// מושרש ב-exeDir. AddJsonFile עם נתיב מוחלט מחזיר בשקט "לא נמצא" (PhysicalFileProvider דוחה
// כל נתיב שאינו מתחת לשורש שלו), כך שה-override לא נטען וה-StorageRoot נשאר בברירת המחדל.
// לכן טוענים אותו דרך FileProvider המושרש בתיקייה שלו עצמו.
var configBuilder = new ConfigurationBuilder()
    .SetBasePath(exeDir)
    .AddJsonFile("appsettings.json", optional: true, reloadOnChange: false);
if (Directory.Exists(overrideDir))
    configBuilder.AddJsonFile(new PhysicalFileProvider(overrideDir), overrideName,
...
// נטען דרך FileProvider המושרש בתיקיית ה-override עצמה: הנתיב מוחלט (ProgramData) ואינו
// מתחת ל-ContentRoot (exeDir), ולכן AddJsonFile רגיל היה מדלג עליו בשקט (וה-StorageRoot
// היה נשאר בברירת המחדל %LOCALAPPDATA% - כך שהדשבורד ב-ProgramData מציג "אין נתונים").
if (Directory.Exists(overrideDir))
    builder.Configuration.AddJsonFile(new PhysicalFileProvider(overrideDir), overrideName,
